namespace Zkly.Admin.Web.Framework
{
    using System;
    using System.Linq;
    ...
    public static class SyndicationFeedExtensions
    {
        ...
        public static void SetThumbnail(this SyndicationItem item, string url, int? width, int? height)
        {
            ...
            item.ElementExtensions.Add(new SyndicationElementExtension(new XElement(ns + "thumbnail", new XAttribute("url", url), width.HasValue ? new XAttribute("width", width) : null, height.HasValue ? new XAttribute("height", height) : null)));
        }
    }
}